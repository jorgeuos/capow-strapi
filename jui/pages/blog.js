...
const PostsList = ({ page, content, posts, locale }) => {
  ...
  const { data } = useSWR(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/posts?pagination[page]=${pageIndex}&pagination[pageSize]=5&sort[0]=posted_at:desc&populate[]=thumbnail`,
    fetcher,
    {
      fallbackData: posts,
    },
...
export async function getStaticProps(locales) {
  ...
  const resultPage = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/slugify/slugs/page/blog?locale=${locale}&${populateString}`,
    );
    if (resultPage.data) {
  ...
  const posts = await fetcher(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/posts?pagination[page]=1&pagination[pageSize]=5&sort[0]=posted_at:desc&populate[]=thumbnail`,
  );
  if (posts.data) {
